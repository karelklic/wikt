...
//===========================================================================
Format1ToFormat2Dialog::Format1ToFormat2Dialog(QWidget *parent)
    : QDialog(parent), _parser(0), _startButtonStarts(true)
{
	ui.setupUi(this);
  ...
  ui.sourceEdit->setText(
      settings.value("format1toFormat2DialogSource", "").toString());
  ui.destinationEdit->setText(
      settings.value("format1toFormat2DialogDestination", "").toString());
}
...
void Format1ToFormat2Dialog::onBrowseSourceButtonClick()
{
  QString fileName = QFileDialog::getOpenFileName(this,
      "Select Format1 (XML Wiktionary) file", QDir::homePath(), "Format1 Files (*.ei1)");
...
void Format1ToFormat2Dialog::onBrowseDestinationButtonClick()
{
  QString fileName = QFileDialog::getSaveFileName(this,
      "New Format2 file", QDir::homePath(), "Format2 Files (*.ei2)");
  if (fileName.length() == 0)
    return;
...
void Format1ToFormat2Dialog::onStartButtonClick()
{
  ...
  if (_startButtonStarts)
  {
    ...
    _parser = new Format1Parser(ui.sourceEdit->text(), ui.destinationEdit->text());
