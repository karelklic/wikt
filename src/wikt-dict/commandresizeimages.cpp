...
struct Image
{
  ...
  Image() {}
  Image(QString filePath, QSize originalSize) :
    _filePath(filePath), _originalSize(originalSize) {}
};

void commandResizeImages(const QString &midFile, const QString &mediaDir)
{
  ...
  for (Format3Reader::EntryMap::const_iterator it = entries.constBegin();
  ...
  out << "Resizing..." << endl;
  for (QMap<QString, Image>::const_iterator it = sizes.constBegin();
       it != sizes.constEnd(); ++it)
  {
    ...
    foreach(QSize size, it.value()._linkSizes)
    {
      if (size.width() < 0) // not valid
        size.setWidth(it.value()._originalSize.width() * size.height() /
                      (double)it.value()._originalSize.height());
      if (size.height() < 0) // not valid
        size.setHeight(it.value()._originalSize.height() * size.width() /
                       (double)it.value()._originalSize.width());
    ...
    int originalPixelCount = it.value()._originalSize.width() *
      it.value()._originalSize.height();
