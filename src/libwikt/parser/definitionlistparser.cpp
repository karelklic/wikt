...
DefinitionListNode *DefinitionListParser::parse(Buffer &buffer)
{
  if (buffer.endOfFile()) return 0;
  if (buffer.nextChar() != ';' && buffer.nextChar() != ':') return 0;
  ...
  {
    // Common line
    if (buffer.nextChar() == ';' || buffer.nextChar() == ':')
    {
  ...
  DefinitionListNode *listNode = new DefinitionListNode();
