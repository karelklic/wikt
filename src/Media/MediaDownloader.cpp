...
//===========================================================================
MediaDownloader::MediaDownloader(const QString& sourceFile,
  const QString& destinationDir) : _sourceFile(sourceFile),
  _destinationDir(destinationDir),
  _pageCounter(0), _downloadsInProgress(0)
{
...
void MediaDownloader::processContent(const QString &name, const QString &content)
{
  ...
  if (!node)
  {
    log("Error parsing \"" + name + "\"");
    CERR("Error parsing \"" + name + "\"");
  }
  else
  {
...
void MediaDownloader::processLink(const LinkNode &node)
{
  ...
  QByteArray urlText = QString("http://en.wiktionary.org/wiki/File:").toAscii() +
